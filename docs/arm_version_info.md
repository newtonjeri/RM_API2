...
Dual-arm setup (Butterfli robot). Both arms queried live over the API on **2026-08-13** using read-only `rm_get_*` calls (RM_API2 SDK 1.1.6, no motion commands issued).

<!-- BEGIN AUTO-VERSIONS -->
## 1. Current versions

*Auto-generated by `docs/update_arm_version_info.py` — last hardware query **2026-08-13 18:28** (read-only getters, no motion commanded).*
...
|---|---|---|---|
| Dynamics collision detection (`collision_stage`) | **level 3** | **level 0** (OFF) | **levels differ ⇒ align them** |

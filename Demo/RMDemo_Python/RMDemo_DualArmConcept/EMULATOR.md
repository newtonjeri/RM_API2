...
| Inspire hand | `rm_set_hand_angle` (6 values 0–1000, −1 = hold), stroke time = 115 ms latency + 373 × span / SPEED_SET ms; **NO device-2 user-callback events (observed on real fw — only the SDK-internal blocking wait sees arrivals)**; blocking returns after the stroke; modbus registers ANGLE_SET/SPEED_SET/ANGLE_ACT/FORCE_ACT emulated with protocol/modbus exclusivity (−5 / silent swallow) | bench §3.7/§3.8 + C7 hardware runs 2026-08-06 |
